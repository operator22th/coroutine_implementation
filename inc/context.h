...
struct basic_context {
  uint64_t *stack; //用户栈
  uint64_t stack_size;
  uint64_t caller_registers[(int)Registers::RegisterCount]; //“内核”寄存器
  uint64_t callee_registers[(int)Registers::RegisterCount]; //“用户”寄存器
  bool finished;
  bool ready;
  ...
  basic_context(uint64_t stack_size)
    ...
    ready_func = [this](){
        //return true;
        return std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count() - clock_set > ms;
    };
